...
#from torch.nn import CrossEntropyLossl
...
def set_interact_args():
    ...
    parser.add_argument('--max_len', type=int, default=600, help='每个utterance的最大长度,超过指定长度则进行截断')
    ...
    parser.add_argument('--test_data', default = 'data/test.csv', type=str, required=False, help='驗證集')
...
def main():
    ...
    rouge = Rouge()
    df_test = pd.read_csv(args.test_data, usecols=['summarization', 'article'])
    df_test["GPT2_summary"] = None
    ##驗證模型的資料導入


    print('***********************Summary model start************************')

    for num_arti, article in enumerate(df_test["article"]):
        try:
            print(article)
            max_score = -1
            best_summary=""
            ...
            for i in range(15):
                ...
                if score[0]["rouge-l"]["f"] > 0.2 or score[0]["rouge-l"]["p"] > 0.3:
                    score_all =  score[0]["rouge-l"]["f"]*0.1 + score[0]["rouge-2"]["p"]*0.4 + score[0]["rouge-l"]["p"]*0.6
            ...
            df_test.loc[num_arti, "GPT2_summary"] = best_summary
